...
public class PlayerDataAccess {
    ...
    private void initialize() {
        players.add(new PlayerDataObject(0, "zero", 0, 0, 0, 0));
        players.add(new PlayerDataObject(1, "one", 0, 0, 0, 0));
        nextId = 2;
    }
    ...
    public static void Save(PlayerDataObject playerToSave) {
        for( PlayerDataObject player : players) {
            if (player.id == playerToSave.id) {
                ...
                player.totalGames = playerToSave.totalGames;
                player.gamesWon = playerToSave.gamesWon;
                player.gamesLost = playerToSave.gamesLost;
                player.placement = playerToSave.placement;
            }
        }
    }
